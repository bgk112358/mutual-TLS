...
#else
	int retval = x509_name_type_from_der(oid, &d, &dlen);
	printf("[bgk] 555 1 retval = %d, tag = %d, oid = %d, dlen = %ld \n", retval, *tag, *oid, dlen);
	if (retval != 1) {error_print();return -1;}

	retval = x509_directory_name_from_der(tag, val, vlen, &d, &dlen);
	printf("[bgk] 555 2 retval = %d, tag = %d, vlen = %d, dlen = %ld\n", retval, *tag, *vlen, dlen);
	if (retval != 1) {error_print();return -1;}

	retval = x509_attr_type_and_value_check(*oid, *tag, *val, *vlen);
	printf("[bgk] 555 3 retval = %d, tag = %d, oid = %d, vlen = %ld \n", retval, *tag, *oid, *vlen);
	if (retval != 1) {error_print();return -1;}

	retval = asn1_length_is_zero(dlen);
	printf("[bgk] 555 4 retval = %d, dlen = %ld \n", retval, dlen);
	if (retval != 1) {error_print();return -1;}
#endif
	...
	int *path_len_constraint)
{
	...
	printf("[bgk] 444 2 issuer = %s issuer_len = %ld \n", issuer, issuer_len);
	...
	const uint8_t *rootcerts, size_t rootcertslen, int depth, int *verify_result)
{
	...
	printf("[bgk] 444 1 certlen, = %d entity_cert_type = %d, path_len_constraint  = %d \n", certlen, entity_cert_type, path_len_constraint);
	...
	while (certslen) {
		...
		printf("[bgk] 333 1 certslen = %d, path_len = %d, path_len_constraint = %d \n", certslen, path_len, path_len_constraint);
		...
		printf("[bgk] 333 2 certslen = %d, path_len = %d, path_len_constraint = %d \n", certslen, path_len, path_len_constraint);
